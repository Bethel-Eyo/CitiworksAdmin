...
export default class ClientJobs extends React.Component {
  constructor() {
    super();
    this.state = {
      jobs: [],
  ...
  componentDidMount() {
    ...
    axios
      .get("http://citiworksapi.test/api/admins/user-jobs", {
        headers: headers
      })
      .then(response => {
        this.setState({
  ...
  handleChange(event, value) {
    ...
    axios
      .get("http://citiworksapi.test/api/admins/user-jobs?page=" + value, {
        headers: headers
      })
      .then(response => {
        this.setState({
        ...
        <GridItem xs={12} sm={12} md={12}>
          <Card>
            <CardHeader color="primary">
              <h4 className={this.classes.cardTitleWhite}>Client Jobs Table</h4>
              <p className={this.classes.cardCategoryWhite}>
                A list of the jobs ordered by all clients
              </p>
            </CardHeader>
            <CardBody>
              <Table
